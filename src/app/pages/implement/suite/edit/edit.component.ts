...
export class SuiteEditComponent implements OnInit, AfterViewInit {
  ...
  save() {
    this._suiteService.save(CONSTANT.CURR_PRJ_ID, this.model).subscribe((json: any) => {
      if (json.code == 1) {
  ...
  editSuiteCases(suite: any, index: number) {
    ...
    this._caseService.queryForSuiteSelection(CONSTANT.CURR_PRJ_ID, suite.id).subscribe((json: any) => {
    ...
    this.caseSelectionModal.result.then((result) => {
      const id = suite ? suite.id : undefined;
      this.saveSuiteCases(id, result.data, index);
    }, (reason) => {
      logger.log('reason', reason);
    });
  }

  saveSuiteCases(suiteId: any, cases: any[], index: number): void {
    if (!this.model.id) {
      this._suiteService.save(CONSTANT.CURR_PRJ_ID, this.model).subscribe((json: any) => {
        if (json.code == 1) {
          ...
          this._saveSuiteCases(this.suiteId, cases);
        } else {
          this.formErrors = [json.msg];
        }
      });
    } else {
      this._saveSuiteCases(suiteId, cases);
    }
  }

  _saveSuiteCases(suiteId: any, cases: any[]) {
    this._suiteService.saveSuiteCases(suiteId, cases).subscribe((json: any) => {
