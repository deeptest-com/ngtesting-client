...
export class ProjectPageEdit implements OnInit, AfterViewInit {
  ...
  constructor(private _state: GlobalState, private _routeService: RouteService, private _route: ActivatedRoute,
              private fb: FormBuilder, private _pageService: IssuePageService) {
  ...
  loadData() {
    ...
    this._pageService.get(this.id).subscribe((json: any) => {
      this.model = json.data;
    });
  }

  save() {
    this._pageService.save(this.model).subscribe((json: any) => {
      if (json.code == 1) {
        this.gotoList();
      } else {
  ...
  delete() {
    this._pageService.delete(this.model.id).subscribe((json: any) => {
      if (json.code == 1) {
  ...
  gotoList() {
    const uri = '/pages/org/' + CONSTANT.CURR_ORG_ID + '/prjs/' + this.projectId + '/config/page/list';
